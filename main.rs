...
use std::thread; // to get threads to spawn
...
fn main() {
	...
	for x in xs {
		handler_list.push(thread::spawn(move || map_data(&x)));
	}
	for t in handler_list {
		let intermediate_sum = t.join().unwrap();
	...
	let mut handler_list2 = vec![];
	for x in xs2 {
		handler_list2.push(thread::spawn(move || map_data(&x)));
	}
	// 4. Collects the intermediate sums from all the threads
    for t in handler_list2 {
		let intermediate_sum = t.join().unwrap();
    ...
    let finalsum = reduce_data(&intermediate_sums2);
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
	...
	let vlen = v.len();
	let mut remaining = vlen%num_partitions;
	for i in 0..num_partitions{
		let mut x_i : Vec<usize> = Vec::new();
		for j in 0..partition_size{
			x_i.push(v[j+i*partition_size]);
		}
		if(remaining > 0){
			x_i.push(v[partition_size+i*partition_size+1]);
			remaining -= 1;
		}
		// Add x1 to the vector that will be returned by this function
		xs.push(x_i);
